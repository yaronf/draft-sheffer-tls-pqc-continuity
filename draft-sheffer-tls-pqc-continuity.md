...
actor strips the PQC or Composite certificate and forces the use of a classical certificate once
...
It MAY appear in the ClientHello (CH) message, and in Certificate (CT) messages sent by either client or server.
...
to indicate support for this mechanism.
...
Note on terminology: Since the extension may be by either client or server,
we use the term "sender" for the peer that sent the
